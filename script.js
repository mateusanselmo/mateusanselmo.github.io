window.onload = () => {
    ...
    var snake = new Snake(10, 10, 1, 0, 5);
    var apple = new Apple(15, 15);
    
    // Stage, Contexto e Ranking
    ...
    function game(){
        ...
        if(snake.pos.x == apple.x && snake.pos.y == apple.y){
            ...
            for(i = 0; i < snake.trail.length; i++) {
                if(snake.trail[i].x == apple.x && snake.trail[i].y == apple.y) {
                    while (snake.trail[i].x == apple.x && snake.trail[i].y == apple.y) {
                        apple.x = Math.floor(Math.random()*tg);
                        apple.y = Math.floor(Math.random()*tg);
                    }
                }
    ...
    function getScore(highscore) {
        for(i = 1; i <= 10; i++){
            highscore.push(document.getElementById(`${i}`).innerText);
        }
    }
